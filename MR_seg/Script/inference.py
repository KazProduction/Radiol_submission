...
from utils.util import save_image, show_slice_img, val_show_slice_img_2
...
test_data, test_label_data = load_data(test_path, test_label_path)
...
test_files = [{"image": img, "label": label}
             for img, label in
             zip(test_data, test_label_data)]
...
if __name__ == "__main__":
    ...
    db_test = MR_seg(data=test_files, transform=transforms.Compose([
                            NormThresholding(up=75, low=20),
                            Padding(),
                            # RandAffined(keys=['image', 'label'],
                            #             mode=['bilinear', 'nearest'],
    ...
    with torch.no_grad():
        for i, val_data in enumerate(testloader):
            ref_name = test_lst[i]
            ref_img = sitk.ReadImage(os.path.join(test_path, ref_name))

            base_name = ref_name.split('.')[0]
            ext = '.'.join(ref_name.split('.')[1:])
            seg_save_name = base_name + "_seg." + ext
            print(seg_save_name)
            save_name = output_path + "/" + seg_save_name
            # print(save_name)
            ...
            print(type(outputs_soft))

            save_image(outputs_soft, ref_img, save_name)
            val_show_slice_img_2(val_img, outputs_soft, 32)
