...
class VTNAffineStem(nn.Module):
    ...
    def __init__(self, dim=1, channels=16, flow_multiplier=1., im_size=512):
        ...
        self.fc_loc = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Dropout(0.5),
            nn.Linear(256, 6*(dim - 1))    # 6 parameters for 2D affine, 12 parameters for 3D affine
        )

        # Initialize the weights/bias with identity transformation
        # The last layer's weights are initialized to zero, and its bias is initialized with an identity transformation.
