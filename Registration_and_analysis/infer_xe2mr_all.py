...
def save_image(img, ref_img, name):
    ...
    sitk.WriteImage(img, os.path.join('Result/xe2mr_all/warped_img', name))
...
def save_ddf(img, ref_img, name):
    ...
    # spacing = (5.0, 5.0, 5.0)
    # img.SetSpacing(spacing)
    sitk.WriteImage(img, os.path.join('Result/xe2mr_all/ddf', name))
...
def show_registered_slices(moving_image, registered_img, fixed_image, slice_num):
    ...
    plt.imshow(registered_img.cpu().squeeze().detach().numpy()[slice_num, :, :], cmap="hot", alpha=0.5)
    plt.show()
...
def main():
    ...
    '''load your model'''
    ckp_path = r'./ckp/xe2mr/epoch_300.pth'
    ...
    with torch.no_grad():
        ...
        #     fixed_name = str(i) + "_fixed.nii.gz"
